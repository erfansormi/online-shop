...
const TextArea = ({ height, error, touched, className, useInForm, ...props }: Props & IProps) => {
    ...
    return (
        <>
            <textarea
                className={`${className ? className : ""} ${styles.input} w-full normal-case rounded-md p-3 bg-gray-200 focus:outline-gray-400 border-0 focus:outline focus:outline-2 ${errorClasses()} resize-y min-h-[100px] max-h-[200px] text-sm`}
                {...props}
            >
            </textarea>
            {
